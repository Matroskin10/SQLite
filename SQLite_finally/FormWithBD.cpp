...
sqlite3_stmt * pStatement_row;
const char * rowTable = "Select count(*) from urls;";
int returnValue = sqlite3_prepare_v2(Database, rowTable, -1, &pStatement_row, NULL);
...
int result1 = sqlite3_step(pStatement_row);
if (result1 == SQLITE_ROW)
{
row = sqlite3_column_int(pStatement_row, 0);
}
else return;
sqlite3_finalize(pStatement_row);
